...
class Config:
    ...
    GENTLE_MAX_CURVATURE = 3.8
    STRAIGHT_STALL_THRESHOLD = 0.70
    SHARP_ENTRY_MAX_SPEED = 2.4
    SHARP_POST_TARGET_SPEED = 4.1
    GENTLE_CORNER_SPEED = 4.25
    BASE_WEAVE_THRESHOLD = 11.0
...
def determine_racing_phase(x, y, waypoints, next_idx, turn_type, curvature):
    ...
    if turn_type == 2:
        ...
        post_start = apex_end
        post_end = post_start + POST_LEN[turn_type]
...
def reward_function(params):
    ...
    if turn_type == 0:
        ...
        if phase == 1:
            ...
            total *= 1.22 + min(speed / Config.SHARP_POST_TARGET_SPEED, 0.58)
    ...
    if phase > 0:
        if phase == 4:
            ...
            tgt = (Config.LateralTarget.POST_OUTSIDE if d_near <= 0 else -Config.LateralTarget.POST_OUTSIDE)
            err = abs(signed_lat - tgt)
            acc = 1.0 - min(err / 0.30, 1.0) ** 2
            total *= (1.0 + Config.SMOOTHNESS_WEIGHT_OTHER * acc)
